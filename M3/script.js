...
function plot_states() {
  d3.json(PATH2)
    ...
    .then((data) => {
      ...
      // render_scatterplot_atts(data, "abs_time", "mod", "att");
    });
}
...
function render_scatterplot_states(
  ...
  allPoints = scatterplot
    ...
    .join((enter) => {
      ...
      //     .scaleSequential()
      //     .domain(d3.extent(atts))
      //     .interpolator(d3.interpolateRgb);

      //   g.append("rect")
      //     .attr("class", "point")
      //     .attr("x", function (d) {
      //       return xScale(d[X_field]) - (rect_dim * 1.3) / 2;
      //     })
      //     .attr("y", function (d) {
      //       return yScale(d[Y_field]) - (rect_dim * 1.3) / 2;
      //     })
      //     .attr("width", ((rect_dim * 1.3) / currentZoom) * 1)
      //     .attr("height", ((rect_dim * 1.3) / currentZoom) * 1)
      //     .attr("fill", function (d) {
      ...
      g.append("rect")
        .attr("class", "point")
        .attr("x", function (d) {
          return xScale(d[X_field]) - rect_dim / 2;
        })
        .attr("y", function (d) {
          return yScale(d[Y_field]) - rect_dim / 2;
        })
...
function render_scatterplot_atts(data, X_field, Y_field, color_field) {
  //   X_field = "intime";
  //   Y_field = "hadm_id";
  //   color_field = "careunit";
  var currentZoom = 1;
  ...
  var colorScale = d3
    ...
    .interpolator(d3.interpolateViridis);
  console.log("EXTENT", d3.extent(names));
  //   render_legend_tsne(colorScale);
  ...
  allPoints = scatterplot
    ...
    .join((enter) => {
      ...
      //   .attr("class", "point")
      //   .attr("x", function (d) {
      //     return xScale(d[X_field]) - rect_dim / 4;
      //   })
      //   .attr("y", function (d) {
      //     return yScale(d[Y_field]) - rect_dim / 4;
      //   })
      //   .attr("width", (rect_dim / currentZoom) * 0.5)
      //   .attr("height", (rect_dim / currentZoom) * 0.5)
      //   .attr("fill", function (d) {
      //     // return colorScale(d[color_field]);
      //     return "green";
      //   });
